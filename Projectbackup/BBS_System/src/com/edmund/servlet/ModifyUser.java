		...
		HttpSession session=request.getSession();
		 ...
		 if(user.getUsername().equals(username)&& user.getPassword().equals(password)){
			if(newpassword.equals(newpassword1)) {
				...
				response.sendRedirect("http://localhost:8080/BBS_System/jsp/ModifyUser.jsp?username=success");
			}else{
				System.out.println("¡Ω¥Œ√‹¬Î≤ª“ª÷¬");
				response.sendRedirect("http://localhost:8080/BBS_System/jsp/ModifyUser.jsp?username=fail");
			}

		 }else{
		//System.out.println("√‹¬Î¥ÌŒÛ");
		 response.sendRedirect("http://localhost:8080/BBS_System/jsp/ModifyUser.jsp?username=exist");
		}
